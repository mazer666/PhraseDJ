...
#[tauri::command]
pub fn mixer_set_fader(deck: u32, value: f32, state: State<'_, AppState>) -> Result<(), String> {
...
#[tauri::command]
pub fn mixer_set_crossfader(value: f32, state: State<'_, AppState>) -> Result<(), String> {
    let guard = state.engine.lock().map_err(|e| e.to_string())?;
    let engine = guard.as_ref().ok_or("audio engine not available")?;
...
#[tauri::command]
pub fn mixer_set_master_gain(value: f32, state: State<'_, AppState>) -> Result<(), String> {
    let guard = state.engine.lock().map_err(|e| e.to_string())?;
    let engine = guard.as_ref().ok_or("audio engine not available")?;
