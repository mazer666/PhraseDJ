...
#[tauri::command]
pub fn library_import_file(path: String, state: State<'_, AppState>) -> Result<String, String> {
...
#[tauri::command]
pub fn library_scan_folder(path: String, state: State<'_, AppState>) -> Result<ScanReport, String> {
    let lib = state.library.lock().map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn library_recent(limit: u32, state: State<'_, AppState>) -> Result<Vec<Track>, String> {
    let lib = state.library.lock().map_err(|e| e.to_string())?;
...
/// Search the library.
#[tauri::command]
pub fn library_search(query: Query, state: State<'_, AppState>) -> Result<Vec<Track>, String> {
    let lib = state.library.lock().map_err(|e| e.to_string())?;
