...
export function Deck({ side }: DeckProps): React.JSX.Element {
  ...
  const stemGains = useEngineStore((s) =>
    side === "A" ? s.stemGainsA : s.stemGainsB,
  );
  ...
  const stemMute = useEngineStore((s) =>
    side === "A" ? s.stemMuteA : s.stemMuteB,
  );
  const toggleStemMute = useEngineStore((s) => s.toggleStemMute);
  const [stemSolo, setStemSolo] = useState<
  ...
  const ratioLabel =
    ratioPct >= 0 ? `+${ratioPct.toFixed(1)} %` : `${ratioPct.toFixed(1)} %`;
    ...
    flushTimer.current = window.setTimeout(() => {
      for (const stem of [0, 1, 2, 3] as const) {
        ...
        const processed = Math.max(
          ...
          Math.min(
            1.5,
            stemGains[stem] * soloGate * muteGate * eqMultiplierForStem(stem),
          ),
        );
  ...
  return (
    <div className={`deck deck-${side.toLowerCase()}`}>
      ...
      <div className="waveform-toolbar">
        ...
        >
          {showBeatGrid ? "Beats: On" : "Beats: Off"}
        </button>
      </div>
      <WaveformCanvas
        waveform={waveform}
      ...
      <div className="deck-transport">
        ...
        <button
          className="btn-secondary"
          onClick={() => autoTransition(autoBeats, deckIndex)}
          disabled={!state.loaded}
        >
          AutoSwitch
        </button>
        <input
          ...
          step={2}
          value={autoBeats}
          onChange={(e) => setAutoBeats(parseInt(e.target.value || "16", 10))}
          title="AutoSwitch beats"
        />
        {state.playing ? (
          <button
      ...
      <div className="deck-stems-block">
        {stemStatus && stemStatus.status !== "cached" && (
          <div className={`stem-status stem-${stemStatus.status}`}>
            {stemStatus.status === "running" ||
            stemStatus.status === "model_downloading"
        ...
        {[
          ...
          <div className="deck-fader-block stem-fader" key={label}>
            ...
            />
            <button
              className={`btn-chip ${stemMute[index] ? "active" : ""}`}
              onClick={() => void toggleStemMute(deckIndex, index)}
            >
              M
            </button>
            <button
              className={`btn-chip ${stemSolo[index] ? "active" : ""}`}
              onClick={() =>
                setStemSolo((m) => {
                  const n = [...m] as [boolean, boolean, boolean, boolean];
                  n[index] = !n[index];
                  return n;
                })
              }
            ...
            <span className="value-label">
              {Math.round(stemGains[index] * 100)}
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="deck-fader-block main-fader">
        ...
        <input
          ...
          step={0.01}
          value={fader}
          onChange={(e) => setFader(deckIndex, parseFloat(e.target.value))}
        />
