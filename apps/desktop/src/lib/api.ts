...
export const app = {
  ...
  settingsSave: (settings: UiSettings) =>
...
export const libraryApi = {
  importFile: (path: string) => invoke<string>("library_import_file", { path }),
  scanFolder: (path: string) =>
    invoke<ScanReport>("library_scan_folder", { path }),
  recent: (limit = 50) => invoke<Track[]>("library_recent", { limit }),
  search: (query: Query) => invoke<Track[]>("library_search", { query }),
};
